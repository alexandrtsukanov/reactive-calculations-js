...
describe('Функции', () => {
    describe('Простая зависимость', () => {
        ...
        const f2 = fromFn(f1).dependFn(val => val + 1);
    
        test('Инициализация f1', () => {
            expect(f1.getFn()(10, 5)).toBe(15);
        })
    
        test('Зависимость f2 от f1', () => {
            expect(f2.getFn()(10, 5)).toBe(16);
        })
    
        test('Зависимость f2 от f1 с другими аргументами', () => {
            expect(f2.getFn()(20, 10)).toBe(31);
        })
    ...
    describe('Длинная цепочка зависимостей', () => {
        const f = (a, b) => a + b + 1;
        const f1 = createFunction(f);
        const f2 = fromFn(f1).dependFn(val => val + 2);
        const f3 = fromFn(f2).dependFn(val => val - 10);
        const f4 = fromFn(f3).dependFn(val => val * 2);

        test('Инициализация f1', () => {
            expect(f1.getFn()(10, 5)).toBe(16);
        })
    
        test('Зависимость f2 от f1', () => {
            expect(f2.getFn()(10, 5)).toBe(18);
        })

        test('Зависимость f3 от f2', () => {
            expect(f3.getFn()(10, 5)).toBe(8);
        })

        test('Зависимость f4 от f3', () => {
            expect(f4.getFn()(10, 5)).toBe(16);
        })
    
        test('Зависимость f4 от f3 с другими аргументами', () => {
            expect(f4.getFn()(9, 10)).toBe(24);
        })
        ...
        test('Обновление f1', () => {
            ...
            expect(f1.getFn()(2, 4)).toBe(10);
        });
    
        test('Обновление f2 после обновления f1', () => {
            expect(f2.getFn()(2, 4)).toBe(12);
        })

        test('Обновление f3 после обновления f1', () => {
            expect(f3.getFn()(2, 4)).toBe(2);
        })

        test('Обновление f4 после обновления f1', () => {
            expect(f4.getFn()(2, 4)).toBe(4);
        })
    })
    ...
    describe('Цепочка правила зависимости', () => {
        ...
        const f2 = fromFn(f1)
            .dependFn(val => val + 1)
            .dependFn(val => val * 2)
            .dependFn(val => val.toString())

        test('Зависимость f2 от f1', () => {
            expect(f2.getFn()(3, 4)).toBe('10');
        });
    ...
    describe('Зависимость от двух или более функций', () => {
        ...
        test('Зависимость f3 от f1 и(или) f2', () => {
            expect(() => fromFn(f1, f2).dependFn((val1, val2) => val1 + val2)).toThrow();
        });
    })
    ...
    describe('Пустая зависимость', () => {
        ...
        const f2 = fromFn(f1);
        const f3 = fromFn(f2).dependFn(num => num * 2);

        test('Инициализация f1', () => {
            expect(f1.getFn()('hello!')).toBe(6);
        });

        test('Зависимость f2 от f1, пустая функция', () => {
            expect(f2.getFn()()).toBeUndefined()
        });
        
        test('Зависимость f3 от f1', () => {
            expect(f3.getFn()('hello!')).toBe(12);
        });
    })
    ...
    describe('Инициализация зависимости в рандомный момент', () => {
        ...
        test('f2 не зависит от f1', () => {
            f1.updateFn((a, b) => a - b + 1);

            expect(f2.getFn()(3, 4)).toBe(10);
        });

        test('f2 зависит от f1', () => {
            f2.dependsOn(f1).dependFn(val => val.toString())

            expect(f2.getFn()(3, 4)).toBe('0');
        });

        test('Обновление f2 после обновления f1', () => {
            f1.updateFn((a, b) => a - b + 2)

            expect(f2.getFn()(3, 4)).toBe('1');
        });
    })
    ...
    describe('Освобождение от зависимостей', () => {
        ...
        test('Зависимость f2 от f1', () => {
            expect(f2.getFn()(4, 5)).toBe(19);
        });

        test('Зависимость f2 от f1', () => {
            f1.free(f2);
            f1.updateFn((a, b) => (a + b) * 3);
